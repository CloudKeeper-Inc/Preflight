...
- Lake Formation data lake admins and grants that name an Identity
  Center (`AWSReservedSSO_*`) role. These live in Lake Formation's own
  permission store rather than in a resource policy, so the Glue catalog
  check above does not cover them.
