...
def _supports_concurrency_mode(client) -> bool:
    """Whether the installed botocore knows `OperationPreferences.ConcurrencyMode`.

    Added in botocore 1.32.0 (boto3 1.29.0); this project's floor is lower.
    An older SDK rejects the key client-side with `ParamValidationError`, which
    is *not* a `ClientError` and would therefore escape `cleanup_stackset`'s
    handler and abort teardown, stranding the assessor role in every member
    account. Cheaper and safer to ask the service model than to catch it.
    """
    try:
        prefs = (
            client.meta.service_model.operation_model("DeleteStackInstances")
            .input_shape.members["OperationPreferences"]
        )
        return "ConcurrencyMode" in prefs.members
    except Exception:  # pragma: no cover - defensive
        return False


def _operation_preferences(client) -> dict:
    """Full-concurrency, never-stop-early preferences for create and delete.

    `FailureTolerancePercentage: 100` is doing most of the work. Under the
    default `STRICT_FAILURE_TOLERANCE`, CloudFormation caps actual concurrency
    at `FailureToleranceCount + 1` regardless of `MaxConcurrentPercentage` — so
    the old tolerance of 10 held a 21-account org to ~3 at a time even though
    it asked for 100%. Raising the tolerance lifts that cap on any SDK version.

    `SOFT_FAILURE_TOLERANCE` adds the rest: under STRICT, concurrency is still
    "reduced proportionally by the number of failures" as the operation runs.
    SOFT decouples the two so a few bad accounts don't re-serialise the batch.

    Tolerating every failure is safe here because neither caller trusts the
    operation's own status: `poll_stack_instances` derives the failed set from
    `expected - enqueued`, and `cleanup_stackset` polls until the instances are
    actually gone. What we lose is CloudFormation stopping early on a bad
    template — which the create call itself would have already rejected.
    """
    prefs: dict = {
        "MaxConcurrentPercentage": 100,
        "FailureTolerancePercentage": 100,
    }
    if _supports_concurrency_mode(client):
        prefs["ConcurrencyMode"] = "SOFT_FAILURE_TOLERANCE"
    return prefs
...
def deploy_stackset(
    ...
    response = cfn.create_stack_instances(
        ...
        OperationPreferences=_operation_preferences(cfn),
        CallAs="SELF",
    )
