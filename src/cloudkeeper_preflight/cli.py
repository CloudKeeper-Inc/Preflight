...
def parse_args(argv: list[str] | None = None) -> argparse.Namespace:
    ...
    parser.add_argument(
        "--skip-member-assessment",
        action="store_true",
        help="Skip Phase 2 (StackSet) and Phase 3 (member account assessment)",
    )
    parser.add_argument(
...
def main() -> None:
    ...
    with ThreadPoolExecutor(max_workers=1, thread_name_prefix="phase1") as p1_executor:
        ...
        phase1_future = p1_executor.submit(
            run_phase1, regions, prefetched=prefetched
        )
        print("[Phase 1/3] Management Account Assessment (running in background)")

        if args.skip_member_assessment:
            print("[Phase 2/3] Skipped (--skip-member-assessment)")
            print("[Phase 3/3] Skipped (--skip-member-assessment)")
        else:
    ...
    if (
        not args.skip_member_assessment
