...
class OrgIdMatcher:
    ...
    IAM Identity Center (`AWSReservedSSO_*`) roles, Identity Center instance or
    application IDs (`ssoins-…`, `apl-…`), or already-deleted IAM role principals
    (`AROA…`) inside arbitrary text or JSON-able policy documents.
    """
    ...
    _ORPHANED_PRINCIPAL_REGEX = re.compile(r"\bAROA[A-Z0-9]{17,}\b")
    _IDC_INSTANCE_REGEX = re.compile(
        r"\b(?:ssoins|apl)-[0-9a-f]{16}\b|\bd-[0-9a-f]{10}\b"
    )
    _CONTEXT_RADIUS = 50
    ...
    def has_match(self, text) -> bool:
        ...
        if self._SSO_ROLE_PATH_REGEX.search(s):
            return True
        if self._ORPHANED_PRINCIPAL_REGEX.search(s):
            return True
        if self._IDC_INSTANCE_REGEX.search(s):
            return True
    ...
    def find_matches(self, text) -> list[dict]:
        ...
        for m in self._ORPHANED_PRINCIPAL_REGEX.finditer(s):
            emit("orphaned_principal", m.group(0), m.start(), m.end())

        for m in self._IDC_INSTANCE_REGEX.finditer(s):
            emit("idc_instance", m.group(0), m.start(), m.end())
