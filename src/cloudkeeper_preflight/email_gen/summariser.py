...
def _build_prism_pitch(ic: dict, reconfig_blocks: list[dict]) -> dict:
    ...
    if ic.get("enabled"):
        ...
        reason = (
            ...
            "org-scoped services below automatically."
        )
    else:
        ...
        reason = (
            ...
            "services below in one click, even without an SSO handoff."
        )
...
def _build_reconfig_blocks(a: dict) -> list[dict]:
        ...
        blocks.append(
            {
                ...
                "doc_ref": None,
                "data": {
                    "application_count": len(enabled_apps),
                    "customer_managed_count": sum(
                        1 for a in enabled_apps if a.get("is_customer_managed")
                    ),
                    "applications": [
                        {
                            "name": app.get("name"),
    ...
    org_hits, sso_hits = _partition_org_references(mor)
    ...
    # from ConfigMap-only clusters, so split into two blocks.
...
_SSO_MATCH_TYPES = frozenset({"sso_role", "sso_role_path"})


def _partition_org_references(mor: dict) -> tuple[dict, dict]:
    """Split resource-policy findings into (org_hits, sso_hits) per service.

    Returns two `{service: finding_count}` dicts. A single finding can land in
    both — a bucket policy that gates on `aws:PrincipalOrgID` *and* names an
    Identity Center role genuinely needs both remediations.

    A finding whose matches carry no recognised type (or no `matches` key at
    all) falls into the org bucket, which is where everything was reported
    ...
    for service, findings in (mor or {}).items():
        ...
        for finding in findings:
            ...
            if (types & _ORG_MATCH_TYPES) or not is_sso:
                org_hits[service] = org_hits.get(service, 0) + 1

    return org_hits, sso_hits
