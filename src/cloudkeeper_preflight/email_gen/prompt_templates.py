...
3. Per-block prose templates for Option C (interpolate the `data` values):
   ...
   - `identity_center_applications` -> "**IdC applications to re-register** - {application_count} enabled application(s) on your current IdC instance ({customer_managed_count} customer-managed, the rest AWS-managed integrations) all need to be re-registered on the new SSO at cutover — nothing transfers automatically. Applications: {each application: name -> assigned_user_count user(s) + assigned_group_count group(s); mark 'AWS-managed' when is_customer_managed is false; note if assignment_required is false ('open to entire identity store')}."
   ...
   - `resource_policy_sso_role_references` -> "**Resource policies pinning Identity Center roles** - {total_hits} resource policy(ies) across {services} name `AWSReservedSSO_*` roles directly. Those role names carry a generated suffix that changes when Identity Center is re-provisioned, so each policy needs repointing at the new role ARNs once Prism is in place."
   - `eks_access_entry_sso_refs` -> "**EKS access entries pinning Identity Center roles** - {cluster_count} cluster(s) have Access Entries that reference `AWSReservedSSO_*` role ARNs directly. Those role names carry a generated suffix that changes when Identity Center is re-provisioned, so each Access Entry needs repointing at the new Prism-generated role ARNs. Clusters: {each cluster: account_id / region / cluster_name; then per SSO reference: principal_arn -> kubernetes_groups}."
   - `eks_configmap_inspection` -> "**EKS clusters using the aws-auth ConfigMap** - {cluster_count} cluster(s) still have the legacy `aws-auth` ConfigMap in the authentication path. AWS APIs cannot enumerate what the ConfigMap contains — your team needs to `kubectl -n kube-system get cm aws-auth -o yaml` on each cluster and update any `AWSReservedSSO_*` mappings to the new Prism role ARNs. Clusters: {each cluster: account_id / region / cluster_name / authentication_mode}."

   For each Option C entry whose `prism_covers=true`, append `*(Prism handles this in one click.)*` on the same line or next line.

4. Option A lists ONLY the block kinds with `prism_covers=true`. Use human names ("IAM Identity Center", "CloudTrail organization trail", "GuardDuty (delegated -> invite-based)", "AWS Config aggregator", "Macie", "Security Hub CSPM", "EKS access entries pinning Identity Center roles", "Identity Center role references in resource policies").
