...
- identity_center_applications -> "Identity Center applications - {application_count} enabled applications on your current Identity Center instance ({customer_managed_count} customer-managed, the rest AWS-managed integrations) must be re-registered on the new SSO at cutover; nothing transfers automatically. {When high_severity_count is greater than 0, open with this sentence before the application list: 'Treat these as cutover blockers, not re-registration work: high_severity_names. For these services the user profiles, home directories and saved work live behind Identity Center and do not come back when the application is re-registered - confirm what each one holds before a cutover date is agreed.'} {When review_count is greater than 0, add: 'review_count AWS-managed application(s) could not be classified automatically and need a manual look.'} Applications: {for each application, semicolon-separated, in the order given: name (assigned_user_count users, assigned_group_count groups); add 'AWS-managed' when is_customer_managed is false; add 'open to entire identity store' when assignment_required is false; add 'HIGH - user state does not survive' when severity is high; add 'unclassified, review' when severity is review}."
- identity_center_coupled_services -> "Services wired directly to Identity Center - {resource_count} resource(s) across {services} were confirmed bound to the current Identity Center instance by querying each service directly. Treat these as cutover blockers rather than re-registration work: the user profiles, home directories and saved work behind them are keyed on the Identity Center identity and do not come back when the service is re-registered. Resources: {for each resource, semicolon-separated: service - resource (account_id, region)}."
...
- resource_policy_orphaned_principals -> "Resource policies naming deleted principals - {total_hits} resource policies across {services} name an IAM role that no longer exists (AWS rewrites the principal to a bare AROA... ID when the role is deleted). These statements are already dead today, independent of this migration - it usually means Identity Center roles were rebuilt at some point and the policies were never cleaned up. Worth reviewing each one now, since the same statements are the ones most likely to be missed again at cutover."
    ...
    "reconfig_blocks": [
        ...
        {"kind": "identity_center_applications", "prism_covers": False, "doc_ref": None,
         "data": {
             ...
             "review_count": 0,
             "high_severity_names": ["SageMaker Studio"],
             "applications": [
                 {"name": "SageMaker Studio", "application_arn": "arn:aws:sso::100000000001:application/ssoins-abc/apl-sm", "application_provider_arn": "arn:aws:sso::aws:applicationProvider/sagemaker", "is_customer_managed": False, "assignment_required": False, "assigned_user_count": 0, "assigned_group_count": 0, "severity": "high"},
                 {"name": "Jira Cloud", "application_arn": "arn:aws:sso::100000000001:application/ssoins-abc/apl-jira", "application_provider_arn": "arn:aws:sso::aws:applicationProvider/custom-saml", "is_customer_managed": True, "assignment_required": True, "assigned_user_count": 4, "assigned_group_count": 1, "severity": "medium"},
                 {"name": "Salesforce Prod", "application_arn": "arn:aws:sso::100000000001:application/ssoins-abc/apl-sf", "application_provider_arn": "arn:aws:sso::aws:applicationProvider/custom-saml", "is_customer_managed": True, "assignment_required": True, "assigned_user_count": 0, "assigned_group_count": 2, "severity": "medium"},
             ],
         }},
