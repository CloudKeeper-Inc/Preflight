...
Turns a summariser output dict into the email markdown ready for analyst
review. Uses the Converse API + the US inference profile in us-east-1.
...
def generate_email(summary: dict, *, client=None) -> tuple[str, dict]:
    ...
    Returns (markdown, usage). `usage` is Bedrock's token counter dict —
    handy for cost tracking during prompt iteration.
    """
