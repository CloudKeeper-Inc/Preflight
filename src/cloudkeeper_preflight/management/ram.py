...
def _assess_region(region: str, session) -> tuple[list[dict], list[dict]]:
    ...
    for share in raw_shares:
        ...
        resources, res_errors = _list_resources(client, region, arn)
        principals, prin_errors = _list_principals(client, region, arn)
...
def _list_resources(client, region: str, share_arn: str) -> tuple[list[dict], list[dict]]:
    try:
...
def _list_principals(client, region: str, share_arn: str) -> tuple[list[dict], list[dict]]:
    try:
        items = paginate_with_token(
            client,
            "list_principals",
