...
Topology:
  ...
  7. Every regional fan-out runs over the *cost-filtered* region list. The
     one exception is the SSO region probe — see `discovery_regions` below.
...
def run_phase1(
    ...
    *,
    ...
    discovery_regions: list[str] | None = None,
    region_selection: dict | None = None,
    region_selection_errors: list[dict] | None = None,
) -> tuple[dict, list[dict]]:
    """Run the management-account assessment modules.

    `regions` is the cost-filtered assessment list from
    `region_costs.select_assessment_regions` — every regional fan-out
    (RAM, config aggregators, resource policies, service configs) uses it.

    `discovery_regions` is the full enabled-region list, used *only* for the
    IAM Identity Center region probe. IdC itself is free, so its home region
    can easily fall under the cost threshold; probing the filtered list would
    report `enabled: False` for an org that does have IdC, and silently
    suppress the SSO-gated EKS charge check with it. Defaults to `regions`.
    ...
    `region_selection` / `region_selection_errors` are the detail dict and
    errors from the region-cost call the CLI already made during bootstrap;
    they're threaded through so they land in the output alongside everything
    else Phase 1 produces.
    ...
    results["region_selection"] = region_selection or {}
    errors.extend(region_selection_errors or [])

    sso_regions = discovery_regions if discovery_regions is not None else regions
    ...
    with ThreadPoolExecutor(max_workers=16) as executor:
        ...
        idc_services_f = (
            executor.submit(
                scan_identity_center_services, regions, matcher, session=session
            )
            if matcher is not None
            else None
        )
        ...
        sso_f = executor.submit(assess_sso, accounts, sso_regions, session=session)
    ...
    if idc_services_f is not None:
        idc_services, idc_errors = idc_services_f.result()
        results["identity_center_services"] = idc_services
        errors.extend(idc_errors)
    else:
        results["identity_center_services"] = {
            "checked": False,
