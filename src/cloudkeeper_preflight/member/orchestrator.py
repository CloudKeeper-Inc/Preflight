...
from cloudkeeper_preflight.member.identity_center_services import (
    scan_member_identity_center_services,
)
...
def run_member_assessments(
    ...
    `org_context` must include:
      ...
      - `regions` (list[str]) — org-wide set, used for any account Cost
        Explorer had no data for
      - `account_regions` (dict[str, list[str]]) — optional per-account
        override, narrower than `regions`
    ...
    account_regions: dict[str, list[str]] = org_context.get("account_regions") or {}
    ...
    def assess_account(account_id: str) -> tuple[str, dict, list[dict]]:
        ...
        # This account's own spend regions when Cost Explorer had data for it,
        # otherwise the org-wide set. Recorded on the result so a reader can
        # tell a narrowed account from one that fell back.
        scan_regions = account_regions.get(account_id) or regions
        account_result["regions_assessed"] = scan_regions
        account_result["region_scope"] = (
            "per_account" if account_regions.get(account_id) else "org_wide"
        )

        with ThreadPoolExecutor(max_workers=5) as executor:
            futures: dict = {}
            futures[executor.submit(scan_member_resource_policies, session, scan_regions, matcher)] = "resource_policies"
            futures[executor.submit(scan_iam, session, matcher)] = "iam"
            futures[executor.submit(scan_member_ram, session, scan_regions)] = "ram"
            futures[executor.submit(scan_member_eks_access, session, scan_regions, matcher)] = "eks_access"
            futures[executor.submit(scan_member_identity_center_services, session, scan_regions, matcher)] = "identity_center_services"
