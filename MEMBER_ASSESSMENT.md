...
- Per-member-account: deep IAM scan (trust + managed + inline), RAM
  ...
  globals), EKS Access Entry enumeration (every cluster's Access Entries
  are checked for `AWSReservedSSO_*` role references, and clusters using
  the legacy `aws-auth` ConfigMap are flagged for manual `kubectl`
  inspection), and delegated-service configuration where the account is
